...
class WriteScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Title
        title = Label(text='Add an Employee Records', color=(0, 0, 0, 1), size_hint=(1, 0.2), font_size=42)
        ...
        anchor_status = AnchorLayout(anchor_x='center', anchor_y='top', size_hint=(1, 0.2))
        ...
        no_option = RadioButton(state='down', group='my_group', size_hint=(0.10, 0.8))
        no_label = Label(text='NO', size_hint=(0.1, 0.5),  color=(0, 0, 0, 1), font_size=32)

        yes_option = RadioButton(group='my_group', size_hint=(0.10, 0.8))
        yes_label = Label(text='YES', size_hint=(0.1, 0.5), color=(0, 0, 0, 1), font_size=32)

        # Store the reference to the 'status' input field
        self.inputs['status'] = yes_option

        # Submit buttons
        anchor_layout_submit = AnchorLayout(anchor_x='center', anchor_y='top', size_hint=(1, 0.22))
        buttonSub_container = BoxLayout(orientation='vertical', size_hint=(0.5, 0.4))
